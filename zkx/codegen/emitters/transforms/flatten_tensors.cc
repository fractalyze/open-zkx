...
// Resolve UnrealizedConversionCastOp between vector types with the same
// element type by replacing with vector::ShapeCastOp. This handles casts
// that pair flatten/unflatten directions but cannot cancel because MLIR's
// builtin cast folding doesn't apply to non-builtin element types.
//
// Note: vector::ShapeCastOp::fold() calls getIntOrFloatBitWidth(), so we
// must skip element types that are neither integer nor float (e.g., field
// types). Those casts are resolved after field type lowering.
struct RewriteVectorUnrealizedCast
  ...
  LogicalResult matchAndRewrite(UnrealizedConversionCastOp op,
    ...
    auto src_type = mlir::dyn_cast<VectorType>(op.getOperand(0).getType());
    auto dst_type = mlir::dyn_cast<VectorType>(op.getResult(0).getType());
    ...
    // ShapeCastOp::fold() assumes IntegerType or FloatType elements.
    // Skip custom types to avoid crashes in getDenseElementBitWidth().
    auto elem = src_type.getElementType();
    if (!mlir::isa<mlir::IntegerType, mlir::FloatType>(elem)) {
      return mlir::failure();
    }
    rewriter.replaceOpWithNewOp<vector::ShapeCastOp>(op, dst_type,
                                                     op.getOperand(0));
    return mlir::success();
  }
};
...
class FlattenTensorsPass
 ...
 public:
  void runOnOperation() override {
    ...
    // integer from shuffle lowering) are not ours — skip them. Also skip casts
    // whose element type is neither integer nor float (e.g., field types):
    // vector::ShapeCastOp::fold() cannot handle them, so they are left as casts
    // and resolved after field type lowering.
    mlir::WalkResult walk_result =
        module.walk([&](UnrealizedConversionCastOp op) {
          ...
          if (!mlir::isa<mlir::IntegerType, mlir::FloatType>(src_elem)) {
            return mlir::WalkResult::advance();
          }
