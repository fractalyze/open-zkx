...
// Verify that field types use field.bitcast around gpu.shuffle.
!pf = !field.pf<2013265921 : i32>

func.func @combiner_pf(%a: !pf, %b: !pf) -> !pf {
  return %a : !pf
}

func.func @shuffler_pf(%a: !pf) -> !pf {
  %ret = zkx_gpu.shuffle_reduce (%a) to 1 combiner=@combiner_pf : !pf
  return %ret : !pf
}
...
// Verify that field-typed materialize uses field.constant for zero init.
!pf = !field.pf<2013265921 : i32>

func.func private @exp_pf(%p0: tensor<4x!pf>, %i: index) -> !pf

#map = #zkx.indexing_map<"(d0)[s0] -> (d0 + s0), domain: d0 in [0, 2], s0 in [0, 1]">
#map1 = #zkx.indexing_map<"(d0)[s0] -> (s0), domain: d0 in [0, 2], s0 in [0, 1]">

func.func @materialize_pf(%input: tensor<4x!pf>, %i: index)
    -> !zkx_gpu.indexed_vector<4x2x!pf, #map1> {
  %0 = zkx_gpu.materialize @exp_pf(%input) at #map(%i)
    : (tensor<4x!pf>) -> !zkx_gpu.indexed_vector<4x2x!pf, #map1>
  func.return %0 : !zkx_gpu.indexed_vector<4x2x!pf, #map1>
}
// CHECK-LABEL: @materialize_pf
// CHECK: %[[INIT:.*]] = field.constant dense<0> : vector<2x!pf_babybear>
// CHECK: zkx.loop {{.*}} iter_args(%{{.*}} = %[[INIT]])
