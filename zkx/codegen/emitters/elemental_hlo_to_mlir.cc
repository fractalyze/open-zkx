...
absl::StatusOr<SmallVector<Value, 1>> EmitBitReverse(
    const HloInstruction *instr, ValueRange indices,
    const OperandProvider &operand_provider, ImplicitLocOpBuilder &b) {
  auto *bit_reverse = Cast<HloBitReverseInstruction>(instr);
  SmallVector<Value, 3> operand_indices(indices.begin(), indices.end());
  auto i64_type = b.getIntegerType(64);

  for (int64_t dim : bit_reverse->dimensions()) {
    int64_t dim_size = instr->shape().dimensions(dim);
    // Compute k where dim_size = 2^k.
    int k = 0;
    for (int64_t s = dim_size; s > 1; s >>= 1) ++k;
    if (k == 0) continue;  // Size 1, bit-reverse is identity.

    // Convert index to i64 for bit manipulation.
    Value idx = b.create<arith::IndexCastUIOp>(i64_type, indices[dim]);
    Value result = b.create<ConstantOp>(b.getIntegerAttr(i64_type, 0));
    Value one = b.create<ConstantOp>(b.getIntegerAttr(i64_type, 1));

    // Unrolled bit-reverse: result |= ((idx >> j) & 1) << (k - 1 - j)
    for (int j = 0; j < k; ++j) {
      Value shift_r = b.create<ConstantOp>(b.getIntegerAttr(i64_type, j));
      Value bit =
          b.create<arith::AndIOp>(b.create<arith::ShRUIOp>(idx, shift_r), one);
      Value shift_l =
          b.create<ConstantOp>(b.getIntegerAttr(i64_type, k - 1 - j));
      result =
          b.create<arith::OrIOp>(result, b.create<arith::ShLIOp>(bit, shift_l));
    }

    // Convert back to index type.
    operand_indices[dim] =
        b.create<arith::IndexCastOp>(b.getIndexType(), result);
  }
...
ValueRange ProvideParameter(const PartitionedComputation &computation,
  ...
  const absl::flat_hash_map<const HloInstruction *, int>
      &injected_value_starts = caller->injected_value_starts;
