      ...
      for (HloComputation *subcomputation :
        ...
        switch (instruction->opcode()) {
          ...
          case HloOpcode::kWhile:
            // Call, conditional, and while operations inherit their
...
absl::StatusOr<int64_t> BufferAssignment::ComputeTotalFragmentationBytes()
    const {
  // Stubbed out: heap_simulator was removed.
  // TODO: Reimplement fragmentation calculation without HeapSimulator.
  return -1;
}
...
std::string BufferAssignment::BufferInfoString() const {
  ...
  std::vector<std::pair<const HloValue *, BufferAllocation::OffsetSize>>
      buffers;
...
absl::Status BufferAssigner::AssignPresetBuffers(
    ...
    BufferAssignment *assignment) {
  // Stubbed out: memory_space_assignment was removed.
  // PresetAssignments is an incomplete type, so preset_assignments_ is always
  // null.
  return absl::OkStatus();
}
...
absl::Status BufferAssigner::AssignBuffersWithSequentialOrdering(
    ...
    GlobalDecreasingSizeBestFitHeap<HloValue>::BufferIntervalCompare
        heap_buffer_interval_compare,
    std::optional<BufferAssignment::BufferIsolationOptions> isolation_options) {
  // HeapSimulator was removed. Group deferred values back into their
  // HloBuffers so that aliased values (e.g. while-loop phi buffers) share a
  // single allocation.
  absl::flat_hash_set<const HloBuffer *> assigned_buffers;
  for (const auto &pair : buffers_to_assign_sequentially) {
    for (const HloValue *value : pair.second) {
      const HloBuffer &buffer =
          assignment->alias_analysis().GetBufferContainingValue(*value);
      if (!assigned_buffers.insert(&buffer).second) {
        continue;  // Already assigned as part of a previous value's buffer.
      }
      if (assignment->HasAllocation(buffer)) {
        continue;  // Already assigned (e.g. by operand reuse).
      }
      int64_t size = 0;
      for (const HloValue *v : buffer.values()) {
        size = std::max(size, assignment->buffer_size_(*v));
      }
      assignment->NewAllocation(buffer, size);
    }
  }
  return absl::OkStatus();
}
