...
//   round-trip pipeline.
bool InlineUnderShardy(HloInstruction* instruction) { return true; }
...
bool InlineStreamAnnotation(HloInstruction* instruction) { return true; }
...
bool CallInliner::IsInlineableCallOp(HloInstruction* instruction) const {
  return instruction->opcode() == HloOpcode::kCall &&
         !instruction->has_backend_config() && InlineUnderShardy(instruction) &&
