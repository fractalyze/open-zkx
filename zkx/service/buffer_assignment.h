...
// Stub type replacing deleted memory_space_assignment module.
...
// Stub types replacing deleted heap_simulator module.
// TODO: Remove these stubs if HeapSimulator usage is fully removed.
template <typename T>
struct GlobalDecreasingSizeBestFitHeap {
  enum Type { kSpatial, kTemporal, kCustom };
  using BufferIntervalCompare = std::function<bool(const T *, const T *)>;
};
...
struct HeapSimulator {
  ...
  template <typename T>
  struct HeapResult {
    absl::flat_hash_map<const T *, Chunk> chunk_map;
    int64_t heap_size = 0;
  };

  template <typename T>
  struct Result {
    std::vector<HeapResult<T>> heap_results;
    int64_t heap_size = 0;
...
class BufferAssigner {
 public:
  ...
  using PrivateStacks =
      absl::flat_hash_map<BufferValue::Color,
