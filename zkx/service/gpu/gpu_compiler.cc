...
// static
absl::StatusOr<Compiler::TargetConfig>
...
GpuCompiler::RunHloPasses(std::unique_ptr<HloModule> module,
  ...
  if (cuda_cc != nullptr && cuda_cc->IsAtLeastAmpere()) {
    // This needs to run after every pass affecting fusions.
  }
...
GpuCompiler::CompileSingleModule(
  ...
  TF_ASSIGN_OR_RETURN(BackendCompileResult result,
                      CompileTargetBinary(module_config, llvm_module,
                                          device_description, relocatable,
                                          debug_module, options, shard_number));
...
absl::StatusOr<GpuCompiler::BackendCompileResult>
GpuCompiler::CompileAndLink(const HloModuleConfig &module_config,
  ...
  if (split_modules) {
    TF_ASSIGN_OR_RETURN(backend_result,
                        CompileAndLink(module->config(), compile_module_results,
                                       gpu_device_info, executor, options,
                                       module));
  } else {
...
GpuCompiler::Export(Executable *executable) const {
  ...
  if (!gpu_executable)
    return absl::InternalError("GpuExecutable is null");
