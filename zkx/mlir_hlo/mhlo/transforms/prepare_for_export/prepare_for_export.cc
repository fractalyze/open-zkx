...
void prepareConstantOp(Operation *op, SplatElementsAttr attr) {
  ...
  if (attr.getNumElements() < 32)
    return;
  // Skip wide integer types (e.g. i256, i1024 for ZK field/EC types).
  // The scalar + broadcast pattern can trigger downstream verification
  // failures in the kernel emitter's flatten_tensors / partitioned emission.
  ...
  // Use resizeSplat (not reshape): we are reducing the element count from N
  // to 1 (scalar), then broadcasting back.  DenseElementsAttr::reshape
  // asserts equal element counts; resizeSplat is the correct API for
  // changing a splat's dimensions.
  auto reshapedAttr =
...
void PrepareForExportPass::runOnOperation() {
  getOperation().walk([&](Operation *op) -> void {
    ...
    if (isa<ReduceOp, ReduceWindowOp, ScatterOp>(op))
