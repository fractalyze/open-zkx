...
InstructionValueSet &
HloDataflowAnalysis::GetInstructionValueSet(const HloInstruction *instruction) {
  DCHECK(value_sets_.contains(instruction))
      << "Instruction " << instruction->ToString() << " not found.";
  return *value_sets_.find(instruction)->second;
}
...
std::optional<ConcatUsageInfo>
ConcatIsEffectivelyElementwise(const HloInstruction &concat,
    ...
    for (const HloInstruction *user : current->users()) {
      ...
      case HloOpcode::kConcatenate: {
        std::optional<ConcatUsageInfo> optional_concat_info =
            ConcatIsEffectivelyElementwise(*user, *current, concat_usage_info);
...
FollowTupleIndirection(const HloInstruction *instruction,
