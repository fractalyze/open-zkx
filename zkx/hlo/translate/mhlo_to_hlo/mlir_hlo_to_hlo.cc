...
mlir::FailureOr<zkx::Shape> ExtractZkxShape(mlir::Operation *op) {
  if (op->getAttrOfType<mlir::StringAttr>(kDefaultLayoutAttrName)) {
    return op->emitError()
           << "string-based zkx_shape attribute not supported (parser removed)";
  } else {
...
// a tuple instruction with `ops`.
zkx::ZkxOp CreateTupleIfMultipleOps(zkx::ZkxBuilder *builder,
                                    llvm::ArrayRef<zkx::ZkxOp> ops) {
  if (ops.size() == 1) {
    return ops[0];
  }
...
class ConvertToHloModule {
 public:
  ...
  LogicalResult Lower(Operation *inst, bool is_entry_function,
                      llvm::ArrayRef<Value> implicit_results,
...
LogicalResult ExportZkxOp(IfOp op, OpLoweringContext ctx) {
  ...
  zkx::ZkxOp true_arg = CreateTupleIfMultipleOps(ctx.builder, true_args);

  // Create the false branch Zkx argument.
  zkx::ZkxOp false_arg = CreateTupleIfMultipleOps(ctx.builder, false_args);
...
LogicalResult ExportZkxOp(CaseOp op, OpLoweringContext ctx) {
  ...
  for (unsigned i = 0; i < branches.size(); ++i) {
    ...
    branch_operands[i] = CreateTupleIfMultipleOps(ctx.builder, args);
...
LogicalResult ExportZkxOp(CreateTokenOp op, OpLoweringContext ctx) {
  return op.emitError("Not implemented for HloOpcode::kAfterAll");
}
...
LogicalResult ConvertToHloModule::Lower(
  ...
  *return_value = zkx::ZkxOp();
  ...
  if (isa<mhlo::ReturnOp, func::ReturnOp>(inst)) {
    return LowerReturn(inst, is_entry_function, implicit_results, builder,
                       value_lowering, return_value, options_);
  }
...
LogicalResult ConvertToHloModule::LowerBasicBlockAsFunction(
  ...
  zkx::ZkxOp return_value;
  for (auto &inst : *block)
    if (failed(Lower(&inst, is_entry_function, implicit_results, builder,
