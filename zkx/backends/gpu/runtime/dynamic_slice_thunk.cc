...
absl::Status DynamicSliceThunk::Prepare(
  ...
  if (offset_as_function_of_indvar_metadata_ != std::nullopt) {
    // NOTE: HloEvaluator has been removed. Cannot evaluate indvar init.
    return absl::UnimplementedError(
        "DynamicSliceThunk with indvar metadata is not supported "
        "(HloEvaluator removed)");
  }
...
absl::Status DynamicSliceThunk::ExecuteOnStream(const ExecuteParams &params) {
  ...
  for (auto [argument_idx, slice] : llvm::enumerate(slices_)) {
    ...
    for (auto [offset_idx, values] : llvm::enumerate(llvm::zip(
      ...
      } else if (std::get_if<HloModule *>(&offset)) {
        // NOTE: HloEvaluator has been removed. Cannot evaluate offset module.
        return absl::UnimplementedError(
            "DynamicSliceThunk offset-as-function-of-indvar is not supported "
            "(HloEvaluator removed)");
  ...
  if (offset_as_function_of_indvar_metadata_ != std::nullopt) {
    // NOTE: HloEvaluator has been removed. Cannot evaluate indvar update.
    return absl::UnimplementedError(
        "DynamicSliceThunk indvar update is not supported "
        "(HloEvaluator removed)");
  }
