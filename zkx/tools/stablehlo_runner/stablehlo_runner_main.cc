...
Flags:
  ...
  --input_json=/path/to.json   JSON file with {"inputs": [[v0,...], [v0,...]]}
...
absl::StatusOr<std::vector<Literal>> ParseInputLiteralsFromJson(
    const HloModule& module, const std::string& json_path) {
  std::ifstream ifs(json_path);
  if (!ifs.is_open()) {
    return absl::NotFoundError(
        absl::StrCat("Cannot open JSON input file: ", json_path));
  }
  nlohmann::json j;
  try {
    ifs >> j;
  ...
  if (!j.contains("inputs") || !j["inputs"].is_array()) {
    return absl::InvalidArgumentError("JSON must contain an \"inputs\" array");
  }
  const auto& inputs_arr = j["inputs"];
  const HloComputation* entry = module.entry_computation();
  if (static_cast<int64_t>(inputs_arr.size()) != entry->num_parameters()) {
    return absl::InvalidArgumentError(absl::StrCat(
        "JSON has ", inputs_arr.size(), " input arrays but module expects ",
        entry->num_parameters(), " parameters"));
  }
  ...
  for (int64_t i = 0; i < entry->num_parameters(); ++i) {
    ...
    const auto& arr = inputs_arr[i];
    if (!arr.is_array()) {
      return absl::InvalidArgumentError(
          absl::StrCat("inputs[", i, "] must be an array"));
    }
    std::vector<std::string> strs;
    strs.reserve(arr.size());
    for (const auto& elem : arr) {
      strs.push_back(elem.dump());
    }
    std::string csv = absl::StrJoin(strs, ",");
    TF_ASSIGN_OR_RETURN(Literal literal, ParseInputLiteral(shape, csv));
...
absl::Status RunStablehlo(const Options& options, const char* module_path) {
  ...
  TF_ASSIGN_OR_RETURN(auto platform, PlatformUtil::GetPlatform("gpu"));
...
int main(int argc, char** argv) {
  ...
  std::vector<tsl::Flag> flag_list = {
      ...
      tsl::Flag("input_json", &options.input_json,
                "JSON file with {\"inputs\": [[v0,...], [v0,...]]} arrays."),
