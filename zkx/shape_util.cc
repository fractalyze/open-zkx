...
int64_t ShapeUtil::SparseArrayDataSize(const Shape &shape) {
  ...
  constexpr int64_t kIndexSize = sizeof(uint32_t);
  constexpr int64_t kValueSize = sizeof(int);
  int64_t nnz = shape.layout().num_nonzeros();
  if (LayoutUtil::IsCSRArray(shape)) {
    // CSR: (num_rows + 1) row pointers + nnz column indices + nnz values
    return (shape.dimensions(0) + 1) * kIndexSize + nnz * kIndexSize +
           nnz * kValueSize;
  } else if (LayoutUtil::IsCSCArray(shape)) {
    // CSC: (num_cols + 1) column pointers + nnz row indices + nnz values
    return (shape.dimensions(1) + 1) * kIndexSize + nnz * kIndexSize +
           nnz * kValueSize;
  } else if (LayoutUtil::IsCOOArray(shape)) {
    CHECK_EQ(shape.layout().dim_level_types_size(), 2)
        << "COO sparse tensor not implemented";
    // COO: nnz row indices + nnz column indices + nnz values
    return nnz * kIndexSize * 2 + nnz * kValueSize;
  }
